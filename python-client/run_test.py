...
"""
requires that this file be ran from the project root directory
"""
...
def load_latencies(config):
    ...
    for i in range(0, 4):
        for load in range(500, 5000, 500):
            output_file_name = config + "load_" + \
                str(load) + "_" + str(i) + ".data"
            command = run_command.split(" ")
            command.extend(["-n", "10000", "-d", "30", "-l",
                           str(load), "-s", "server0:5000"])
...
def file_size_latencies(config):
    ...
    for file_size in range(1000, 100000, 10000):
        ...
        command = run_command.split(" ")
        command.extend(["-n", str(file_size), "-l",
                       "50", "-s", "server0:5000"])
...
def analyze_file_size_latencies(config):
    ...
    for file_size in range(1000, 100000, 10000):
        output_file_name = config + "file_size_" + \
            str(file_size) + "_" + ".data"
...
def main():
    ...
    load_latencies("bm_")
    load_to_latencies, load_to_completed_requests = analyze_load_latencies(
        "bm_")
